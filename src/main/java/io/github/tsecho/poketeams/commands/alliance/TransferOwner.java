...
public class TransferOwner implements CommandExecutor {
    ...
    public CommandResult execute(CommandSource src, CommandContext args) throws CommandException {
        ...
        if(role.getTeam().equals(roleOther.getTeam()))
            return ErrorCheck.test(src, ErrorMessage.CANT_BE_SAME_TEAM);
        if((!alliance.inAlliance() || !allianceOther.inAlliance()) || !alliance.getAlliance().equals(allianceOther.getAlliance()))
            return ErrorCheck.test(src, ErrorMessage.BOTH_NOT_IN_ALLIANCE);
        ...
        alliance.getAllTeams().stream()
                ...
                .forEach(user -> user.getPlayer().ifPresent(player -> player.sendMessage(SuccessMessage.TRANSFERRED_ALLIANCE.getText(src)))));
